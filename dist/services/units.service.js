...
let UnitsService = UnitsService_1 = class UnitsService {
    constructor(unitsRepo, pusherService, cacheService) {
        ...
        this.NOTIFICATION_COOLDOWN_MS = 10000;
    ...
    async registerUnitUltraFast(rfid, scannerCode, additionalData = {}) {
        ...
        if (existingUnit) {
            ...
            return Object.assign(Object.assign({}, this.cleanUnitResponse(cleanedUnit)), { _existingRfid: true, _updatedToDelivered: true, _transactionId: transactionId, _predictiveSentAt: predictiveSentAt, _totalLatency: totalTime });
        }
        if (!scanner) {
            this.sendRegistrationFailed(rfid, transactionId, "Scanner not found");
            throw Error("Registration scanner not found");
    ...
    async executeMinimalTransaction(rfid, scanner, additionalData) {
        return await this.unitsRepo.manager.transaction(async (entityManager) => {
            ...
            const savedUnit = await entityManager.save(Units_1.Units, unit);
    ...
    async updateExistingUnitToDelivered(existingUnit, scanner, transactionId) {
        return await this.unitsRepo.manager.transaction(async (entityManager) => {
            ...
            const isAlreadyDelivered = ((_a = unit.location) === null || _a === void 0 ? void 0 : _a.locationId) === deliveredLocation.locationId &&
                ((_b = unit.status) === null || _b === void 0 ? void 0 : _b.statusId) === deliveredStatus.statusId;
            ...
            this.cacheService.del(cache_constant_1.CacheKeys.units.byCode(updatedUnit.unitCode));
            ...
            const pusherData = {
                ...
                _scannedAt: ((_c = scanner.location) === null || _c === void 0 ? void 0 : _c.name) || 'Unknown Scanner Location'
            };
    ...
    clearCacheImmediately(rfid, unitCode) {
        const unitCacheKey = this.keyUnit(rfid);
        const unitCodeCacheKey = cache_constant_1.CacheKeys.units.byCode(unitCode);
    ...
    async handleAsyncPostRegistration(unit, scanner) {
        try {
            const unitLogs = new UnitLogs_1.UnitLogs();
            ...
            await this.unitsRepo.manager.save(UnitLogs_1.UnitLogs, unitLogs);
        }
        catch (error) {
            this.logger.warn(`Async post-registration failed: ${error.message}`);
        }
    }
    sendRegistrationFailed(rfid, transactionId, error) {
            ...
            for (const log of logsDto.data) {
                ...
                if (scanner.scannerType === "REGISTRATION") {
                    ...
                    try {
                        ...
                        if (!deliveredLocation && locations_constant_1.FIXED_LOCATIONS.DELIVERED.id) {
                            deliveredLocation = await entityManager.findOne(Locations_1.Locations, {
                                where: { locationId: locations_constant_1.FIXED_LOCATIONS.DELIVERED.id, active: true },
                            });
                        }
                        if (!deliveredLocation) {
                        ...
                        const deliveredStatus = await entityManager.findOne(Status_1.Status, {
                            where: { statusId: status_constants_1.STATUS.DELIVERED.toString() },
                        });
                        if (!deliveredStatus) {
                            this.logger.error(`DELIVERED status not found for RFID: ${rfid}`);
                            throw Error(`DELIVERED status (ID: ${status_constants_1.STATUS.DELIVERED}) not found in database.`);
                        }
                        const isAlreadyDelivered = ((_r = unit.location) === null || _r === void 0 ? void 0 : _r.locationId) === deliveredLocation.locationId &&
                            ((_s = unit.status) === null || _s === void 0 ? void 0 : _s.statusId) === deliveredStatus.statusId;
                        if (isAlreadyDelivered) {
